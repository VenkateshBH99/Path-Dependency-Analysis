...
class CFG(object):
    ...
    def _visit_blocks(self, graph, block, visited=[], calls=True):
        ...
        Dict_node[str(block.id)]=nodelabel
        ...
        for exit in block.exits:
            ...
            value=str(block.id)+"_"+str(exit.target.id)
            if value not in Dict_edge.keys():
                Dict_edge[value]=[]
            Dict_edge[value].append(edgelabel)
            # print("edge:",edgelabel," ",value)
